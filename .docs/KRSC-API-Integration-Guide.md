...
The full name of KRSC is the Korean Seller Center. It is a seller center for Korean cross-border sellers. Sellers can manage the products, orders, and marketing of multiple shops through it. For the basic operation guide and introduction of KRSC, please click here.
...
a. Please refer to this article to find out what type of developer account you are. Login to Open Platform >> Console >> App List >> Select developer type >> Add

\*Different types of developers will own different types of apps. For details, please refer to App management page.
...
b. When the seller logs in to KRSC and completes the upgrade for each shops’ products, v2.shop.get_shop_info will return mtsku_upgraded_status:UPGRADED

c.Test Global Product API and Merchant API. You can check the API call flow from here

2.6 Merchant & Shop Authorization
Because after the shop account is upgraded to KRSC, multiple shops will belong to merchants, you can learn more about the relationship between Merchant and shop through this FAQ. Therefore, in order for you to call the GlobalProduct API normally, please re-authorize your shops and switch the sub account page when authorizing.
...
3）It is necessary to ensure that the APP status is Online, so that seller users can use the KRSC API in the production environment. Clicking Go Live in the Open Platform Console and submitting the information. APP will automatically be approved after 24 hours.
...
5）You can learn more KRSC API FAQ from here: KRSC API FAQ
